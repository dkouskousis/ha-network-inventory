const TEXT = {
  en: {
    ...
    labels: "Labels", allLabels: "All labels", labelSettings: "Home Assistant labels", labelSettingsHelp: "Shared with Home Assistant. Add labels here or in HA; rename and delete them from HA Settings.", networkDetails: "Network details",
    ...
    applyField: "Change", labelOperation: "Label operation", keepLabels: "Do not change labels", addLabels: "Add labels",
    removeLabels: "Remove labels", replaceLabels: "Replace labels", bulkUpdated: "devices updated", bulkHelp: "Only checked fields will be changed.",
    fieldOptions: "Dropdown fields", fieldOptionsHelp: "Manage device types, brands, and Home Assistant labels.", configureIntegration: "Configure integration",
    ...
    batteryHelp: "Devices with a Battery label or an assigned battery entity.", replacementSaved: "Battery replacement recorded",
    ...
    confirmFirmwareUpdate: "Install firmware update?", firmwareUpdateStarted: "Firmware update started",
    releaseNotes: "Release notes", integrationsHelp: "Connections used by Network Inventory and their current Home Assistant status.",
    nativeIntegration: "Native Home Assistant integration", shellyDetected: "Detected", shellyNotDetected: "Not configured",
    shellyNativeHelp: "Uses the official Shelly integration already configured in Home Assistant. No Shelly Cloud key is required.",
    shellySetupHelp: "Add the Shelly integration in Home Assistant to detect devices and manage supported firmware updates.",
    ...
    reolinkDetected: "Detected", reolinkNotDetected: "Not configured",
    reolinkNativeHelp: "Uses the official Reolink integration already configured in Home Assistant. No Reolink cloud account is required.",
    reolinkSetupHelp: "Add the Reolink integration in Home Assistant to detect cameras and manage supported firmware updates.",
    addReolink: "Add Reolink to Home Assistant", openReolink: "Open Reolink in Home Assistant", reolinkDevices: "Reolink devices",
    noReolinkFirmwareEntities: "No firmware update entities were found for the configured Reolink devices.",
  ...
  el: {
    ...
    labels: "Labels", allLabels: "Όλα τα labels", labelSettings: "Home Assistant labels", labelSettingsHelp: "Κοινά με το Home Assistant. Πρόσθεσε labels εδώ ή στο HA· μετονομασία και διαγραφή γίνεται από τα Settings του HA.", networkDetails: "Στοιχεία δικτύου",
    ...
    applyField: "Αλλαγή", labelOperation: "Ενέργεια labels", keepLabels: "Να μην αλλάξουν τα labels", addLabels: "Προσθήκη labels",
    removeLabels: "Αφαίρεση labels", replaceLabels: "Αντικατάσταση labels", bulkUpdated: "συσκευές ενημερώθηκαν", bulkHelp: "Θα αλλάξουν μόνο τα επιλεγμένα πεδία.",
    fieldOptions: "Πεδία dropdown", fieldOptionsHelp: "Διαχείριση τύπων συσκευής, brands και Home Assistant labels.", configureIntegration: "Ρύθμιση integration",
    ...
    batteryHelp: "Συσκευές με label Battery ή συνδεδεμένο entity μπαταρίας.", replacementSaved: "Η αλλαγή μπαταρίας καταγράφηκε",
    ...
    confirmFirmwareUpdate: "Να εγκατασταθεί η ενημέρωση firmware;", firmwareUpdateStarted: "Η ενημέρωση firmware ξεκίνησε",
    releaseNotes: "Σημειώσεις έκδοσης", integrationsHelp: "Οι συνδέσεις που χρησιμοποιεί το Network Inventory και η κατάστασή τους στο Home Assistant.",
    nativeIntegration: "Native integration του Home Assistant", shellyDetected: "Βρέθηκε", shellyNotDetected: "Δεν έχει ρυθμιστεί",
    shellyNativeHelp: "Χρησιμοποιεί το επίσημο Shelly integration που είναι ήδη ρυθμισμένο στο Home Assistant. Δεν απαιτείται Shelly Cloud key.",
    shellySetupHelp: "Πρόσθεσε το Shelly integration στο Home Assistant για εντοπισμό συσκευών και διαχείριση των υποστηριζόμενων firmware updates.",
    ...
    reolinkDetected: "Βρέθηκε", reolinkNotDetected: "Δεν έχει ρυθμιστεί",
    reolinkNativeHelp: "Χρησιμοποιεί το επίσημο Reolink integration που είναι ήδη ρυθμισμένο στο Home Assistant. Δεν απαιτείται Reolink cloud account.",
    reolinkSetupHelp: "Πρόσθεσε το Reolink integration στο Home Assistant για εντοπισμό καμερών και διαχείριση των υποστηριζόμενων firmware updates.",
    addReolink: "Προσθήκη Reolink στο Home Assistant", openReolink: "Άνοιγμα Reolink στο Home Assistant", reolinkDevices: "Συσκευές Reolink",
    noReolinkFirmwareEntities: "Δεν βρέθηκαν firmware update entities για τις ρυθμισμένες συσκευές Reolink.",
...
const FILTER_URL_KEYS = {
  ...
  areaFilter: "ni_area", statusFilter: "ni_status", ipFilter: "ni_ip", labelFilter: "ni_label",
...
class NetworkInventoryPanel extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="app">
        <header>
          <div><div class="title-row"><h1>${this.t("title")}</h1><span class="version">v${esc(this.data.version)}</span>${availableUpdate ? `<a class="update-available" href="/config/updates" title="${this.t("viewUpdate")}"><ha-icon icon="mdi:update"></ha-icon>${this.t("updateAvailable")}${availableUpdate.latestVersion ? ` · v${esc(availableUpdate.latestVersion)}` : ""}</a>` : ""}</div></div>
  ...
  batteryDevices() {
    return this.data.devices.filter(device =>
      Boolean(device.battery_entity_id) || (device.labels || []).some(label => label.toLowerCase() === "battery")
    );
  }
  ...
  renderDevices() {
    ...
    const labelOptions = (this.data.labels || []).map(value => option(value, this.labelFilter)).join("");
    ...
    return `
      ...
      <section class="filters card">
        ...
        <select id="label-filter"><option value="">${this.t("allLabels")}</option>${labelOptions}</select>
  ...
  deviceCell(device, key) {
    ...
    if (key === "name") return `<span class="device-name-cell"><strong title="${esc(device.name)}">${esc(device.name)}</strong>${device.ha_device_kind === "child" ? `<ha-icon class="child-device-icon" icon="mdi:file-tree-outline" title="${this.t("childDevice")}"></ha-icon>` : ""}${this.data.unifi_matches?.[device.id] ? `<span class="unifi-dot" title="${this.t("unifiBadge")}"></span>` : ""}</span>`;
  ...
  labelChips(labels = []) {
    ...
    return labels.length ? `<div class="label-list">${visible.map(label => `<span>${esc(label)}</span>`).join("")}${labels.length > 3 ? `<span title="${esc(labels.slice(3).join(", "))}">+${labels.length - 3}</span>` : ""}</div>` : "";
  }
  ...
  renderIntegrations() {
    ...
    return `<section class="section-head"><div><h2>${this.t("integrations")}</h2><p>${this.t("integrationsHelp")}</p></div></section>
      <article class="card integration-card shelly-card">
        ...
        <div class="grow"><div class="integration-title"><h2>Shelly</h2><span class="native-badge">${this.t("nativeIntegration")}</span><span class="status-dot ${shelly.configured ? "ok" : ""}">${shelly.configured ? this.t("shellyDetected") : this.t("shellyNotDetected")}</span></div>
          <p class="muted">${shelly.configured ? this.t("shellyNativeHelp") : this.t("shellySetupHelp")}</p>
          ${shelly.configured ? `<div class="integration-metrics"><div><strong>${esc(shelly.device_count || 0)}</strong><span>${this.t("shellyDevices")}</span></div><div><strong>${esc(shelly.firmware_entity_count || 0)}</strong><span>${this.t("firmwareEntities")}</span></div><div class="${shelly.updates_available ? "attention" : ""}"><strong>${esc(shelly.updates_available || 0)}</strong><span>${this.t("availableUpdates")}</span></div></div>${!shelly.firmware_entity_count ? `<p class="integration-notice"><ha-icon icon="mdi:information-outline"></ha-icon>${this.t("noFirmwareEntities")}</p>` : !shelly.active_firmware_entity_count ? `<p class="integration-notice"><ha-icon icon="mdi:cog-off-outline"></ha-icon>${this.t("disabledFirmwareEntities")}</p>` : ""}` : ""}
        </div>
        <div class="integration-actions"><a class="secondary" href="${shelly.configured ? "/config/integrations/integration/shelly" : "/config/integrations/dashboard/add?domain=shelly"}"><ha-icon icon="${shelly.configured ? "mdi:open-in-new" : "mdi:plus"}"></ha-icon>${this.t(shelly.configured ? "openShelly" : "addShelly")}</a></div>
      </article>
      <article class="card integration-card integration-gap reolink-card">
        <div class="integration-logo reolink-logo"><img src="https://brands.home-assistant.io/_/reolink/icon.png" alt="Reolink"></div>
        <div class="grow"><div class="integration-title"><h2>Reolink</h2><span class="native-badge">${this.t("nativeIntegration")}</span><span class="status-dot ${reolink.configured ? "ok" : ""}">${reolink.configured ? this.t("reolinkDetected") : this.t("reolinkNotDetected")}</span></div>
          <p class="muted">${reolink.configured ? this.t("reolinkNativeHelp") : this.t("reolinkSetupHelp")}</p>
          ${reolink.configured ? `<div class="integration-metrics"><div><strong>${esc(reolink.device_count || 0)}</strong><span>${this.t("reolinkDevices")}</span></div><div><strong>${esc(reolink.firmware_entity_count || 0)}</strong><span>${this.t("firmwareEntities")}</span></div><div class="${reolink.updates_available ? "attention" : ""}"><strong>${esc(reolink.updates_available || 0)}</strong><span>${this.t("availableUpdates")}</span></div></div>${!reolink.firmware_entity_count ? `<p class="integration-notice"><ha-icon icon="mdi:information-outline"></ha-icon>${this.t("noReolinkFirmwareEntities")}</p>` : !reolink.active_firmware_entity_count ? `<p class="integration-notice"><ha-icon icon="mdi:cog-off-outline"></ha-icon>${this.t("disabledFirmwareEntities")}</p>` : ""}` : ""}
        </div>
        <div class="integration-actions"><a class="secondary" href="${reolink.configured ? "/config/integrations/integration/reolink" : "/config/integrations/dashboard/add?domain=reolink"}"><ha-icon icon="${reolink.configured ? "mdi:open-in-new" : "mdi:plus"}"></ha-icon>${this.t(reolink.configured ? "openReolink" : "addReolink")}</a></div>
      </article>
      <article class="card integration-card integration-gap">
        <div class="integration-logo unifi-logo"><img src="https://brands.home-assistant.io/_/unifi/icon.png" alt="UniFi"></div>
  ...
  async undoLog(logId, button) {
    if (!confirm(this.t("undoLogConfirm"))) return;
    button.disabled = true;
    try {
      await this._hass.callWS({ type: "network_inventory/undo_log", log_id: logId });
      await this.reload(this.t("undoCompleted"));
    } catch (error) {
  ...
  bindBaseEvents() {
    ...
    [["protocol", "protocolFilter"], ["type", "typeFilter"], ["brand", "brandFilter"], ["area", "areaFilter"], ["status", "statusFilter"], ["ip", "ipFilter"], ["label", "labelFilter"]].forEach(([id, property]) => {
    ...
    this.shadowRoot.querySelector("[data-action='clear-filters']")?.addEventListener("click", () => {
      this.protocolFilter = this.typeFilter = this.brandFilter = this.areaFilter = this.statusFilter = this.ipFilter = this.labelFilter = "";
      this.query = "";
      this.activeSavedView = "";
      this.syncFiltersToUrl();
      this.render();
  ...
  openBulkEditModal() {
    ...
    const labelOptions = (this.data.labels || []).map(label => `<label><input type="checkbox" name="bulk_labels" value="${esc(label)}">${esc(label)}</label>`).join("");
    const modal = this.shadowRoot.querySelector("#modal");
    modal.innerHTML = `<div class="modal-backdrop"><section class="modal"><div class="modal-head"><div><h2>${this.t("bulkEdit")}</h2><p>${this.selectedDevices.size} ${this.t("selected")} · ${this.t("bulkHelp")}</p></div><button type="button" data-close><ha-icon icon="mdi:close"></ha-icon></button></div>
      <form id="bulk-edit-form"><div class="bulk-form">
        ...
        <div class="bulk-labels"><label>${this.t("labelOperation")}<select name="label_mode"><option value="">${this.t("keepLabels")}</option><option value="add">${this.t("addLabels")}</option><option value="remove">${this.t("removeLabels")}</option><option value="replace">${this.t("replaceLabels")}</option></select></label><div class="bulk-label-options disabled">${labelOptions}</div></div>
    ...
    const labelMode = modal.querySelector("[name='label_mode']");
    labelMode.addEventListener("change", () => {
      const enabled = Boolean(labelMode.value);
      modal.querySelector(".bulk-label-options").classList.toggle("disabled", !enabled);
      modal.querySelectorAll("[name='bulk_labels']").forEach(input => input.disabled = !enabled);
    });
    modal.querySelectorAll("[name='bulk_labels']").forEach(input => input.disabled = true);
  ...
  async saveBulkEdit(event) {
    ...
    const label_mode = form.elements.label_mode.value;
    const labels = [...form.querySelectorAll("[name='bulk_labels']:checked")].map(input => input.value);
    this.setBusy(form, true);
    try {
      const result = await this._hass.callWS({ type: "network_inventory/bulk_update", device_ids: [...this.selectedDevices], fields, label_mode, labels });
  ...
  openDeviceModal(device = null, isImport = false) {
    ...
    const selectedLabels = new Set(device?.labels || []);
    const labelOptions = (this.data.labels || []).map(label => `<label><input type="checkbox" name="labels" value="${esc(label)}" ${selectedLabels.has(label) ? "checked" : ""}>${esc(label)}</label>`).join("");
...
function csvToDevices(text) {
  ...
  return rows.slice(1).map(row => {
    ...
    item.labels = item.labels ? item.labels.split(";").map(label => label.trim()).filter(Boolean) : [];
    return item;
